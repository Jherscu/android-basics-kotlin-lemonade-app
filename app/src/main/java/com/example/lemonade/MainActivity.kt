...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Initialize activity with proper text, content description, and image
        setViewElements()
    ...
    private fun clickLemonImage() {
        ...
        when (lemonadeState) {
            // Initialize lemon squeezing portion of app
            SELECT -> {
            ...
            // Enable lemon to be squeezed until empty
            // When lemon is found to be empty, change state to DRINK
            SQUEEZE -> {
                if (lemonSize != 0) {
                    squeezeCount++
                    lemonSize--
            ...
            // Enable process to reset
            RESTART -> {
                lemonadeState = SELECT
            }
        }

        // Call to set UI based on state changes
        setViewElements()
        ////////////////////// TO DO LIST //////////////////////
        /*=(CHECK)=*/
    ...
    private fun setViewElements() {
        ...
        // Track lemonade state with 'when' conditional and update img, contentDescription,
        // and textAction string accordingly
        when (lemonadeState) {
            SELECT -> {
                lemonImage?.apply {
                    contentDescription = getString(R.string.lemon_select)
                    setImageResource(R.drawable.lemon_tree)
                }
                textAction.text = getString(R.string.lemon_select)
            }
            SQUEEZE -> {
                // If UI has not already been updated, change values
                if (lemonImage?.contentDescription != getString(R.string.lemon_squeeze)) {
                    lemonImage?.apply {
                        contentDescription = getString(R.string.lemon_squeeze)
            ...
            DRINK -> {
                lemonImage?.apply {
                    contentDescription = getString(R.string.lemon_drink)
            ...
            RESTART -> {
                lemonImage?.apply {
                    contentDescription = getString(R.string.lemon_empty_glass)
